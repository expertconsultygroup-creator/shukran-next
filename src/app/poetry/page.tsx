  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] pb-24 relative">
      ...
      <section className="pt-24 pb-16 px-4">
        <div className="container mx-auto max-w-4xl">
          <div className="text-center mb-12">
            <h1 className="font-sans font-black text-3xl sm:text-5xl text-[var(--gold)] drop-shadow-[var(--glow-gold)] mb-4">قصائد في حب الوطن</h1>
            <p className="text-[var(--muted-light)] text-lg">أبيات سُطرت بماء الذهب تقديراً واعتزازاً</p>
          </div>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative bg-[var(--card-glass)] backdrop-blur-xl border border-[var(--gold)] p-8 md:p-16 rounded-2xl shadow-[var(--glow-gold)] text-center">
            <span className="inline-block px-4 py-1 rounded-full bg-[var(--gold-dim)] text-[var(--gold)] font-bold text-sm mb-6 border border-[var(--gold)]">قصيدة اليوم</span>
            <h2 className="font-sans font-bold text-3xl text-[var(--white)] mb-2">{featured.title}</h2>
            <p className="text-[var(--gold-light)] font-sans mb-10 text-lg">الشاعر: {featured.poet}</p>
      ...
      <section className="container mx-auto px-4 max-w-5xl">
        <h3 className="font-sans font-bold text-2xl text-[var(--white)] mb-8 border-r-4 border-[var(--gold)] pr-4">ديوان الشكر</h3>
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-[var(--muted)] text-sm">الشاعر: {poem.poet}</p>
                  </div>
                  {poem.audio_url && <span className="bg-[var(--surface-2)] p-2 rounded-full text-[var(--gold)]"><Play className="w-4 h-4 ml-0.5" fill="currentColor" /></span>}
                </div>
                <div className="font-serif text-[var(--muted-light)] leading-[2.2] text-lg italic opacity-80 mb-6">{poem.text.split('\n').slice(0, 2).join('\n')}...</div>
                <button onClick={() => setExpandedPoem(poem.id)} className="text-[var(--gold)] font-bold text-sm hover:underline">اقرأ كاملاً ←</button>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <button onClick={() => setIsSubmitModalOpen(true)} className="fixed bottom-8 right-8 z-40 bg-[var(--gold)] text-[var(--bg-deep)] px-6 py-4 rounded-full font-sans font-bold text-lg shadow-[var(--glow-gold)] hover:scale-105 active:scale-95 transition-transform flex items-center gap-2">
        <Plus size={24} /> أضف قصيدتك
      </button>
          ...
          return (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-[var(--bg-deep)] border border-[var(--gold)] rounded-2xl p-8 md:p-12 w-full max-w-3xl max-h-[85vh] overflow-y-auto shadow-[var(--glow-gold)]">
                <button onClick={() => setExpandedPoem(null)} className="absolute top-4 right-4 p-2 text-[var(--muted)] hover:text-[var(--white)] rounded-full bg-[var(--surface)]"><X size={24}/></button>
                <div className="text-center">
                  <h2 className="font-sans font-bold text-3xl text-[var(--gold)] mb-2">{p.title}</h2>
                  <p className="text-[var(--muted-light)] mb-10">الشاعر: {p.poet}</p>
        ...
        {isSubmitModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="relative bg-[var(--surface)] border border-[var(--gold-dim)] rounded-2xl p-6 md:p-10 w-full max-w-2xl shadow-2xl">
              <button onClick={() => setIsSubmitModalOpen(false)} className="absolute top-4 right-4 p-2 text-[var(--muted)] hover:text-[var(--white)]"><X size={24}/></button>
              <h2 className="font-sans font-bold text-2xl text-[var(--white)] mb-6">مشاركة قصيدة وطنية</h2>
              <form className="space-y-5" onSubmit={e => { e.preventDefault(); setIsSubmitModalOpen(false); }}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div><label className="block text-[var(--muted)] text-sm mb-2">الاسم / اللقب</label><input type="text" className="w-full bg-[var(--bg-deep)] border border-[var(--border)] rounded-xl text-[var(--white)] px-4 py-3 focus:outline-none focus:border-[var(--gold)]" required /></div>
                  <div><label className="block text-[var(--muted)] text-sm mb-2">عنوان القصيدة</label><input type="text" className="w-full bg-[var(--bg-deep)] border border-[var(--border)] rounded-xl text-[var(--white)] px-4 py-3 focus:outline-none focus:border-[var(--gold)]" required /></div>
                </div>
                <div><label className="block text-[var(--muted)] text-sm mb-2">القصيدة</label><textarea rows={8} className="w-full bg-[var(--bg-deep)] border border-[var(--border)] rounded-xl text-[var(--white)] px-4 py-3 focus:outline-none focus:border-[var(--gold)] font-serif resize-none leading-loose text-center" placeholder="أدخل أبيات القصيدة هنا..." required></textarea></div>
                <button type="submit" className="w-full h-14 rounded-xl bg-[var(--gold)] text-[var(--bg-deep)] font-sans font-bold text-lg mt-6 hover:bg-[var(--gold-light)] transition-colors">إرسال القصيدة للمراجعة</button>
              </form>
            </motion.div>
          </div>
        )}
