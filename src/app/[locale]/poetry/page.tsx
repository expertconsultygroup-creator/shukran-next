...
export default function Poetry() {
  ...
  const recorder = useVoiceRecorder(120);
  ...
  async function handleSubmitPoem(e: React.FormEvent) {
    ...
    try {
      ...
      if (recorder.audioBlob) {
        try {
          audio_url = await uploadAudio(recorder.audioBlob);
        } catch {
          toast.error(isRtl ? "فشل رفع التسجيل الصوتي، حاول مرة أخرى" : "Failed to upload audio, please try again");
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center">
        <div className="w-12 h-12 border-4 rounded-full animate-spin" style={{ borderColor: "var(--surface-2)", borderTopColor: "var(--gold)" }} />
      </div>
    );
  }
      ...
      <section className="pt-24 pb-16 px-4">
        <div className="container mx-auto max-w-4xl">
          ...
          {featured ? (
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative bg-[var(--card-glass)] backdrop-blur-xl border border-[var(--gold-dim)] p-12 md:p-20 rounded-2xl text-center">
              <div className="text-6xl mb-6">📜</div>
              <h2 className="font-sans font-bold text-2xl text-[var(--white)] mb-4">{isRtl ? "لا توجد قصائد بعد" : "No poems yet"}</h2>
              <p className="text-[var(--muted-light)] mb-8 max-w-md mx-auto">{isRtl ? "كن أول من يشارك قصيدة وطنية في حب الوطن وحماته" : "Be the first to share a patriotic poem honoring the nation and its guardians"}</p>
              <button onClick={() => setIsSubmitModalOpen(true)} className="inline-flex items-center gap-2 bg-[var(--gold)] text-[var(--bg-deep)] px-8 py-4 rounded-full font-sans font-bold text-lg shadow-[var(--glow-gold)] hover:scale-105 active:scale-95 transition-transform">
            ...
            {rest.map((poem: any, i: number) => (
              <motion.div key={poem.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-[var(--card-glass-alt)] backdrop-blur-md border border-[var(--border)] rounded-2xl overflow-hidden relative group">
                ...
                <div className="p-6">
                  ...
                  {poem.audio_url && (
                    <div className="mb-4"><AudioPlayer src={poem.audio_url} /></div>
                  )}
      ...
      <AnimatePresence>
        ...
        {isSubmitModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 text-start" dir="auto">
            ...
            <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="relative bg-[var(--surface)] border border-[var(--gold-dim)] rounded-2xl p-6 md:p-10 w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
              <button onClick={() => { setIsSubmitModalOpen(false); recorder.deleteRecording(); }} className="absolute top-4 end-4 p-2 text-[var(--muted)] hover:text-[var(--white)]"><X size={24} /></button>
              ...
              <form className="space-y-5" onSubmit={handleSubmitPoem}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-[var(--muted)] text-sm mb-2">{t("nameLabel")}</label>
                    <input type="text" className="w-full bg-[var(--bg-deep)] border border-[var(--border)] rounded-xl text-[var(--white)] px-4 py-3 focus:outline-none focus:border-[var(--gold)] text-start" required value={formData.poet} onChange={(e) => setFormData({ ...formData, poet: e.target.value })} dir="auto" />
                  </div>
                  <div>
                    <label className="block text-[var(--muted)] text-sm mb-2">{t("poemTitleLabel")}</label>
                    <input type="text" className="w-full bg-[var(--bg-deep)] border border-[var(--border)] rounded-xl text-[var(--white)] px-4 py-3 focus:outline-none focus:border-[var(--gold)] text-start" required value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} dir="auto" />
                  </div>
                </div>
                <div>
                  <label className="block text-[var(--muted)] text-sm mb-2">{t("poemTextLabel")}</label>
                ...
                <div className="pt-4 border-t border-[var(--border)]">
                  <label className="block text-[var(--muted)] text-sm mb-4">{isRtl ? "أضف إلقاء صوتي للقصيدة (اختياري)" : "Add voice recitation (optional)"}</label>
                  {recorder.error && (
                    <p className="text-[var(--red)] text-sm mb-3">{isRtl ? "تم رفض إذن الميكروفون" : "Microphone permission denied"}</p>
                  )}
                  {!recorder.audioUrl && !recorder.isRecording ? (
                    <div className="flex flex-col items-center justify-center py-6">
                      <button type="button" onClick={recorder.startRecording} className="w-16 h-16 rounded-full bg-[var(--surface-2)] border-2 border-[var(--gold-dim)] flex items-center justify-center text-[var(--gold)] hover:scale-105 hover:bg-[var(--gold-dim)] transition-all shadow-[var(--glow-gold)] mb-3">
                        <Mic size={28} />
                      </button>
                      <span className="text-[var(--muted)] text-sm">{isRtl ? "اضغط لتسجيل الإلقاء" : "Click to record recitation"}</span>
                    </div>
                  ) : recorder.isRecording ? (
                    <div className="flex flex-col items-center justify-center py-6">
                      <button type="button" onClick={recorder.stopRecording} className="w-16 h-16 rounded-full bg-[var(--red-dim)] border-2 border-[var(--red)] flex items-center justify-center text-[var(--red)] hover:scale-105 transition-all animate-pulse mb-4 relative">
                        <div className="w-6 h-6 bg-[var(--red)] rounded-sm" />
                      </button>
                    ...
                    <div className="py-4 flex items-center justify-between bg-[var(--surface-2)] px-4 rounded-xl border border-[var(--border)]">
                      ...
                      <button type="button" onClick={recorder.deleteRecording} className="text-[var(--red-light)] text-sm font-bold mx-4 hover:underline">{isRtl ? "حذف" : "Delete"}</button>
                    </div>
                  ) : null}
                </div>

                <button type="submit" disabled={isSubmitting} className="w-full h-14 rounded-xl bg-[var(--gold)] text-[var(--bg-deep)] font-sans font-bold text-lg mt-6 hover:bg-[var(--gold-light)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  {isSubmitting ? "..." : t("submitPoem")}
                </button>
              </form>
            </motion.div>
          </div>
