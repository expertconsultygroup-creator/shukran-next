  ...
  useEffect(() => {
    async function fetchMessages() {
      ...
      const params = new URLSearchParams({
        page: page.toString(),
        limit: "20",
        ...(filter !== t("filterAll") && { category: apiFilter }),
    ...
    fetchMessages();
  }, [filter, searchTerm, page, t, tSend]);

  return (
    <div className="min-h-screen bg-[var(--bg-deep)] relative" dir="auto">
      <ShamsaPattern className="opacity-5" />

      <div className="container mx-auto px-4 py-12 relative z-10 text-start">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <div className="flex items-center gap-4 mb-4">
              <h1 className="font-sans font-black text-3xl sm:text-5xl text-[var(--white)]">{t("title")}</h1>
              <div className="bg-[var(--gold-dim)] border border-[var(--gold)] px-3 py-1 rounded-full flex items-center gap-2">
                ...
                <span className="text-[var(--gold)] font-sans font-bold text-sm tracking-wide">{count.toLocaleString()} {t("messageLabel")}</span>
              </div>
            </div>
            <p className="text-[var(--muted-light)] text-lg max-w-xl">{t("subtitle")}</p>
          </div>
        </div>
        ...
        <div className="sticky top-24 z-40 bg-[var(--overlay-glass)] backdrop-blur-[20px] border border-[var(--border)] rounded-2xl p-4 mb-10 shadow-lg flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="relative w-full md:w-80">
            <Search className="absolute end-4 top-1/2 -translate-y-1/2 text-[var(--muted)]" size={18} />
            <input
              type="text"
              placeholder={t("searchPlaceholder")}
              value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
              className="w-full bg-[var(--bg-deep)] border border-[var(--border)] rounded-xl text-[var(--white)] pe-12 ps-4 py-2.5 focus:outline-none focus:border-[var(--gold)] transition-colors text-start"
              dir="auto"
            />
          </div>
          ...
          <>
            ...
            {messages.length === 0 && !loading && (
              <div className="py-20 text-center">
                ...
                <h3 className="text-xl text-[var(--white)] font-bold mb-2">{t("noResults")}</h3>
                <p className="text-[var(--muted)]">{t("tryChanging")}</p>
              </div>
            )}
