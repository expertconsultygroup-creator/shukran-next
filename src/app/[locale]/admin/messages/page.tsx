...
export default function MessagesAdmin() {
  ...
  async function handleAction(id: string, status: "approved" | "rejected") {
    try {
      ...
      toast.success(status === "approved" ? t("msgApproved") : t("msgRejected"));
    ...
    <div className="space-y-4" dir="auto">
      ...
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-[var(--border)] flex flex-wrap gap-4 items-center justify-between">
          <h3 className="font-bold text-[var(--white)] text-xl">{t("manageMessages")}</h3>
          <div className="flex gap-3">
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-[var(--muted)]">{t("noMessages")}</div>
        ) : (
            ...
            <table className={`w-full ${isRtl ? 'text-right' : 'text-left'} text-sm`}>
              <thead className="bg-[var(--surface-2)] text-[var(--muted)]">
                <tr>
                  ...
                  <th className="px-6 py-3 font-normal">{t("uuid")}</th>
                  <th className="px-6 py-3 font-normal">{t("sender")}</th>
                  <th className="px-6 py-3 font-normal w-[30%]">{t("message")}</th>
                  <th className="px-6 py-3 font-normal">{t("country")}</th>
        ...
        <div className="p-4 border-t border-[var(--border)] text-[var(--muted)] text-sm flex justify-between">
          <span>{t("showing", { count: messages.length, total: total })}</span>
          ...
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-8 max-w-lg w-full text-start" onClick={(e) => e.stopPropagation()}>
            ...
            {previewMsg.status === "pending" && (
              <div className="flex gap-3 mt-6">
                <button onClick={() => { handleAction(previewMsg.id, "approved"); setPreviewMsg(null); }} className="flex-1 py-2.5 rounded-xl bg-[var(--green)] text-white font-bold hover:opacity-90 transition-opacity">{t("approve")}</button>
                <button onClick={() => { handleAction(previewMsg.id, "rejected"); setPreviewMsg(null); }} className="flex-1 py-2.5 rounded-xl bg-[var(--red)] text-white font-bold hover:opacity-90 transition-opacity">{t("reject")}</button>
              </div>
            )}
          </div>
        </div>
