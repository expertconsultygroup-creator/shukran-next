...
export default function MapPage() {
  ...
  // Build a lookup map: country_code -> { name_ar, count }
  const countryMap = useMemo(() => {
    const map: Record<string, { name_ar: string; count: number }> = {};
    countries.forEach((c) => {
      map[c.code] = { name_ar: c.name_ar, count: c.count || 0 };
    });
    return map;
  }, [countries]);
  ...
  function getTooltip(geoName: string) {
    ...
    return { name: info.name_ar, count: info.count };
  }
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
          <div>
            ...
            <p className="text-[var(--muted-light)]">{participatingCountries} دولة شاركت في توثيق حب الوطن</p>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="xl:col-span-1 bg-[var(--surface)] border border-[var(--border)] rounded-3xl p-6 flex flex-col h-[500px] md:h-[600px]">
            <h3 className="font-sans font-bold text-2xl text-[var(--white)] mb-6">أعلى الدول مشاركة</h3>
              ...
              {countries.filter(c => c.count > 0).slice(0, 15).map((country: any, idx: number) => (
                <div key={country.code} className={`flex items-center gap-4 p-3 rounded-xl transition-colors ${idx === 0 ? "bg-[var(--gold-dim)] border border-[var(--gold-dim)]" : "hover:bg-[var(--surface-2)]"}`} style={idx === 0 ? { borderRight: "3px solid var(--gold)" } : {}}>
                  ...
                  <div className="flex-1">
                    <div className="font-sans font-bold text-[var(--white)] text-sm mb-1">{country.name_ar}</div>
                  ...
                  <div className="font-mono text-[var(--muted-light)] text-sm font-bold">{country.count.toLocaleString()}</div>
                </div>
              ))}
              {countries.filter(c => c.count > 0).length === 0 && (
                <div className="text-center text-[var(--muted)] py-12">لا توجد مشاركات بعد</div>
              )}
            </div>
          </div>
        </div>
